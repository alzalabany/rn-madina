...
class CreatePage extends Component {
  ...
  update(label, value){
    ...
    const shouldShowSave = hasChanges.length > 0 ? true : false;
  ...
  render() {
    ...
    return (
        <View 
          ...
          />}
          onLayout={event=>this.setState({width:(event.nativeEvent.layout.width*.5)-20})} style={[styles.card,this.props.style,{flex:1,backgroundColor:'rgba(144,75,120,.6)',padding:0,margin:-10}]}>
...
const state = (state,props) =>({
  visit: state.get(['visits',props.id]),
  user:  state.get(['users',visit.ref_id]),
  room:  state.get(['rooms',visit.room_id]),
});
