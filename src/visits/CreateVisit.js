...
import {
  ...
  View,
  ScrollView,
  Image,Modal,Button,RefreshControl,
...
const User = connect((state,props)=>({user:state.users[props.id]}))(props =>props.render(props.user||{}))
...
class CreatePage extends Component {
  ...
  constructor(props){
      ...
      this.state={
        ...
        config:{},
        drsearchFilter:'',
        ref_id:this.props.currentUser.role==='dr' ? this.props.currentUser.id : null,
        loadingUsers:null,
    ...
    return (
        ...
        <ScrollView onLayout={event=>this.setState({width:(event.nativeEvent.layout.width*.5)-20})} style={this.props.style ? this.props.style : styles.card}>
          ...
          <View style={{flexDirection:'row','flexWrap':'wrap'}}>
            ...
            {this.state.dates.map(time=><TouchableOpacity key={time} onPress={()=>this.changeDate(time)} style={[styles.option,{width:Math.max(this.state.width),flex:undefined}]} 
              >
          ...
          <DateTimePicker
            minimumDate={this.props.currentUser.role==='dr' ? new Date() : undefined}
            date={date.toDate()}
          ...
          onRequestClose={()=>this.setState({showDrSelect:false})}>
            ...
            <ScrollView 
              ...
              <TouchableOpacity key={d.id} style={{borderBottomWidth:1,borderBottomColor:'purple'}}
...
const p = state =>{
  return {
    currentUser: state.getIn(['users',state.user_id]),
    doctors: state.get('users').filter(i=>i.role==='dr'),
  }
}
