...
class Home extends Component {
  ...
  formatVisits(visitsProp,uid, role, filters){
    let {patient, day, room_id} = this.props.filters;
    ...
    visits = visits.filter(visit=>{
      if(role==='admin' || role === 'staff'){
        ...
        if ( visit.user_id !== uid ) return false;
      }
  ...
  render(){
    ...
    return <View style={{flex:1}}>
      ...
      <DateTimePicker
        ...
        {!!Search && <View>
        {Boolean(date && date.format('YYYY-MM-DD')===day) ?
        ...
        {!!Search && <View style={{height:40, justifyContent:'center',alignItems:'center',backgroundColor:'purple',paddingHorizontal:10,paddingVertical:5}}>
          <TextInput
            ...
            placeholder="Search patients.." style={{height:30,flex:1,backgroundColor:'white',borderRadius:10,paddingHorizontal:30}}/>
        </View>
        }

        <Statistics stats={stats} visits={visits} />


        <Tabs selected={this.state.selectedTab}
              titles={tabs} label={i=>typeof i === 'object' ? i.name : i}
              onChange={this.tabChange.bind(this)}
              style={{minHeight:40}}/>

        {Boolean(this.props.filters.room_id && this.props.role!=='dr') ?
